class Line < ApplicationRecord
  ...
  scope :find_title, -> (question){ joins(:poem).where(text: question).pluck("title").join }
  ...
  scope :find_sorted_string, -> (question){joins(:poem).where(sorted_line: question).pluck("sorted_line").join}
end
